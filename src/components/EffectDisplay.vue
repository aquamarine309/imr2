...
export default {
  ...
  computed: {
    ...
    effectDisplay() {
      const effect = this.formatEffect(this.reachedCap ? this.cap : this.effectValue);
      if (effect instanceof Object) {
        if (effect.softcapped) return `${effect.value}<span class="o-softcap">(softcapped)</span>`;
        return effect.value;
      }
      return effect;
    }
    ...
    config: {
      ...
      handler(config) {
        ...
        if (isNumber(value)) {
          ...
          this.updateEffect = () => this.effectValue = effect().copy();
        } else {
...
<template>
  <span v-if="isVisible && effectDisplay !== undefined">
    <br v-if="br">
    {{ labelDisplay }}<span v-html="effectDisplay" />
  </span>
</template>
