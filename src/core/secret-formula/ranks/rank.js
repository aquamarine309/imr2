...
export const rank = {
  ...
  requirement(amount) {
    return Decimal.pow10(
      amount.timesEffectOf(RankType.tier.unlocks.reduceRankReq).pow(1.15)
    ).times(10);
  },
  bulk(value) {
    if (value.lt(10)) return DC.D0;
    return value.div(10).log10().root(1.15)
      .dividedByEffectOf(RankType.tier.unlocks.reduceRankReq).add(1).floor();
  },
  get noReset() {
